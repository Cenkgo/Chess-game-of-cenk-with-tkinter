...
def fnk(a,b):
	def dnd():
		...
		if not scl:
			bt=False
			if tt[b][a]["text"]=="p" and tt[b][a]["fg"]=="white":
				if b==6:
					if tt[b-1][a]["text"]=="":
				...
				if b-1>=0 and a-1>=0 and tt[b-1][a-1]["text"]!="":
					if tt[b-1][a-1]["fg"]=="black":
						...
						bt=True
				scl=bt
			if tt[b][a]["text"]=="p" and tt[b][a]["fg"]=="black":
				if b==1:
					if tt[b+1][a]["text"]=="":
						tt[b+1][a].config(bg="red")
						bt=True
			...
			if tt[b][a]["bg"]=="red":
				...
				for i in range(8):
					for e in range(8):
						...
						tt[i][e].config(bg=rnk)
					scl=False
					if tt[b][a]["text"]=="p" and tt[b][a]["fg"]=="white" and b==0:
						fr2.pack()
						x1,y1=a,b
					if tt[b][a]["text"]=="p" and tt[b][a]["fg"]=="black" and b==7:
						fr2.pack()
						x1,y1=a,b
	btt=[]
	for e in range(8):
...
def fnk2():
	if dg.get()=="at":
		tt[y1][x1].config(text="a")
	if dg.get()=="vezir":
		tt[y1][x1].config(text="v")
	if dg.get()=="kale":
		tt[y1][x1].config(text="k")
	if dg.get()=="fil":
		tt[y1][x1].config(text="f")
	fr2.pack_forget()
fr.pack(pady=(50,0))
for i in range(8):
	for e in range(8):
...
art=0
for i in range(8):
	tt[1][art].config(text="p",fg="black")
	tt[6][art].config(text="p",fg="white")
	art+=1
